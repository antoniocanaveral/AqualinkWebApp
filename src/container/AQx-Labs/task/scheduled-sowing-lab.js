...
function ScheduleSowingLab({ data }) {
  console.log('ScheduleSowingLab props:', { data });
  ...
  const dataSource = eventState.map((value, index) => {
    const {
      ...
      SM_PlantingDate = 'N/A', // Fecha
      id,
    } = value;

    const day = getDay(SM_PlantingDate);
    const monthName = getMonthName(SM_PlantingDate);
...
ScheduleSowingLab.propTypes = {
  data: PropTypes.arrayOf(
    PropTypes.shape({
      ...
      SM_PlantingDate: PropTypes.string, // Fecha de siembra
      id: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
    })
  ).isRequired,
};

export default ScheduleSowingLab;
