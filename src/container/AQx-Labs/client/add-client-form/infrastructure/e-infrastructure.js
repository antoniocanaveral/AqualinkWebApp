...
export const TanqueEngordeInfrastructure = ({
  index,
  alimentadores,
  handleAddAlimentador,
  form,
}) => {
  ...
  useEffect(() => {
    form.setFieldsValue({
      [`identificadorTanque-${index}`]:
        form.getFieldValue(`identificadorTanque-${index}`) || undefined,
      [`dimensionesLargo-${index}`]:
        form.getFieldValue(`dimensionesLargo-${index}`) || undefined,
      [`dimensionesAncho-${index}`]:
        form.getFieldValue(`dimensionesAncho-${index}`) || undefined,
      [`profundidad-${index}`]:
        form.getFieldValue(`profundidad-${index}`) || undefined,
      [`nivelSiembra-${index}`]:
        form.getFieldValue(`nivelSiembra-${index}`) || undefined,
      [`nivelCosecha-${index}`]:
        form.getFieldValue(`nivelCosecha-${index}`) || undefined,
      [`aireacionMecanica-${index}`]:
        form.getFieldValue(`aireacionMecanica-${index}`) || undefined,
      [`metodoAlimentacion-${index}`]:
        form.getFieldValue(`metodoAlimentacion-${index}`) || undefined,
    });
  }, [form, index]);

  // Función de formatter para campos con dos decimales
  ...
  // Función de parser para campos con dos decimales
  const twoDecimalParser = (value) => value.replace(/,/g, "");
      ...
      <Row gutter={16}>
        <Col span={12}>
          <Form.Item
            ...
            rules={[
              { required: true, message: "Ingrese el identificador del tanque" },
            ]}
          >
        ...
        <Col span={12}>
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Ingrese la potencia de aireación mecánica",
              },
              { type: "number", min: 0.01, message: "Debe ser mayor a 0" },
            ]}
          >
      ...
      <Row gutter={16}>
        <Col span={12}>
          <Form.Item
            ...
            rules={[
              { required: true, message: "Seleccione un método de alimentación" },
            ]}
          >
            <Select
            ...
            <Row gutter={16} key={`alimentador-${index}-${alimentadorIndex}`}>
              <Col span={6}>
                <Form.Item
                  ...
                  rules={[
                    {
                      required: true,
                      message: "Ingrese el número del alimentador",
                    },
                  ]}
                >
                  <InputNumber min={1} style={{ width: "100%" }} />
              ...
              <Col span={6}>
                <Form.Item
                  ...
                  rules={[
                    { required: true, message: "Seleccione una marca" },
                  ]}
                >
