...
export const PreCriaInfrastructure = ({ index, alimentadores, handleAddAlimentador, form }) => {
    ...
    useEffect(() => {
        form.setFieldsValue({
            ...
            [`extensionPreCria-${index}`]: form.getFieldValue(`extensionPreCria-${index}`) || 0, // Definir un valor mínimo por defecto
            [`profundidadOperativa-${index}`]: form.getFieldValue(`profundidadOperativa-${index}`) || 0,
            [`profundidadSiembra-${index}`]: form.getFieldValue(`profundidadSiembra-${index}`) || 0,
            [`profundidadTransferencia-${index}`]: form.getFieldValue(`profundidadTransferencia-${index}`) || 0,
