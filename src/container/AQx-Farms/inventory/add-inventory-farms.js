...
function AddInventoryFarms() {
  ...
  // Maneja el cambio de categoría principal y reinicia los estados dependientes
  ...
  // Maneja el cambio en el selector de Grupo
  const handleGroupChange = (value) => {
    ...
    // Filtrar productos por categoría y grupo seleccionado
    let products = [];
  ...
  const handleProductSelect = (value) => {
    ...
    // Filtrar clasificaciones basadas en rsu_code
    let classifications = [];
    if (selectedCategory === 'all') {
  ...
  return (
    <>
      <PageHeader highlightText="Aqualink Camaroneras" title="Inventario" />
      <Main>
        <Row gutter={25}>
          <Col xl={24} xs={24}>
