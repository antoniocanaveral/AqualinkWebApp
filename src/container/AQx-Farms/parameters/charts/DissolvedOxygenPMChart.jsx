...
function DissolvedOxygenNightChart({ selectedBatch }) {
  ...
  const data = [
    {
      x: dayjs(record.fecha_noche).format("DD/MM/YYYY"),
      oxigeno: record.oxig_noche,
    },
  ];
  ...
  const renderTooltip = ({ label, payload }) => {
    ...
    return (
      <div style={{ backgroundColor: "white", padding: "10px", border: "1px solid #ccc" }}>
        ...
        <p style={{ color: "#ff4d4f" }}>{`Rango Crítico: 4.5`}</p>
      </div>
    );
  };
  ...
  return (
    <div>
      ...
      <br />
      <Typography.Text level={4}>Oxígeno Disuelto (Noche)</Typography.Text>
      <ResponsiveContainer width="100%" height={300}>
        ...
        >
          ...
          <ReferenceLine y={4.5} stroke="#ff4d4f" strokeWidth={2} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
...
export default DissolvedOxygenNightChart;