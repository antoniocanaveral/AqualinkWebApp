...
function AlcalinityChart({ selectedBatch }) {
  ...
  const renderTooltip = ({ label, payload }) => {
    ...
    return (
      <div style={{ backgroundColor: "white", padding: "10px", border: "1px solid #ccc" }}>
        <p>{`Fecha: ${label}`}</p>
        <p style={{ color: "#26b99a" }}>{`Alcalinidad: ${payload[0].value}`}</p>
        <p style={{ color: "#a5aeb5" }}>{`Rango Mínimo: 100`}</p>
        <p style={{ color: "#ff4d4f" }}>{`Rango Crítico: 180`}</p>
      </div>
    );
  };
  ...
  return (
    <div>
      ...
      <br />
      <Typography.Text level={4}>Alcalinidad</Typography.Text>
      <ResponsiveContainer width="100%" height={300}>
        ...
        >
          ...
          <ReferenceLine y={180} stroke="#ff4d4f" strokeWidth={2} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
