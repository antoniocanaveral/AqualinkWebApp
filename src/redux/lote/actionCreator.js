...
export const fetchLotesBySM_Coordination_ID = (SM_Coordination_ID) => async (dispatch) => {
  console.log('Fetching lotes for SM_Coordination_ID:', SM_Coordination_ID);
  dispatch(fetchLotesLoading());

  try {

    const response = await DataService.get(`/models/sm_lote?$filter=SM_Coordination_ID eq ${SM_Coordination_ID}`);
    if (response.data && response.data.records) {
      const lotes = response.data.records;
      dispatch(fetchLotesSuccess(lotes));
    } else {
      throw new Error('No se encontraron lotes.');
    }
  } catch (error) {
